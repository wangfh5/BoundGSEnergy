...
An exploratory fixed-depth `N = 30`, `n_super = 5` screen remains `OPTIMAL` and has a positive raw gain of `2.1437405645086116e-7`, but its strict gain is `-1.3332406440951713e-5` because the accumulated moment residual grows. Deeper normalized `N = 30`, `n_super = 8` screens also solve all three matched routes to `OPTIMAL` and increase the raw gain to about `1.5e-6`, but their strict gains remain negative (`-3.30e-5` at coefficient scale `512` and `-3.58e-5` at `16384`). These are not formal finite-size certificates. The remaining blocker is the strict replay loss from ordinary moment equalities, so larger runs cannot establish scalability until an equivalent numerical formulation passes this `N = 30` gate.
